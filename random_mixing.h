...
void MixEvents(int centrality_or_ntrkoff_int, int nEvt_to_mix, std::vector<int> ev_multiplicity, std::vector<double> vtx_z_vec, std::vector<int> ev_phpos, float vzcut, std::vector<std::vector<ROOT::Math::PtEtaPhiMVector>> Track_Vector, std::vector<std::vector<int>> Track_Chg_Vector, std::vector<std::vector<double>> Track_Eff_Vector, THnSparseD *histo_SS, THnSparseD *histo_SSLCMS, THnSparseD *histo_SS3D, THnSparseD *histo_OS, THnSparseD *histo_OSLCMS, THnSparseD *histo_OS3D, bool docostdptcut, bool do_hbt3d, bool dogamovcorrection, int systematic, TH1I* NeventsAss){
   ...
   for(int nevt_trg = 0; nevt_trg < aux_n_evts; nevt_trg++){ // first loop over all events
      ...
      for (int nevt_assoc = assloop_start; nevt_assoc < aux_n_evts; nevt_assoc++){
         ...
         if(ev_phpos[nevt_trg] != ev_phpos[nevt_assoc]) continue; // same photon side
		 if(fabs(ev_multiplicity[nevt_trg] - ev_multiplicity[nevt_assoc]) > centrality_or_ntrkoff_int) continue; // similar ev_multiplicity
         if(fabs(vtx_z_vec[nevt_trg]) > 7.0 && fabs(vtx_z_vec[nevt_trg]) <= 10.0) vzcut = 2.0 * vzcut;
